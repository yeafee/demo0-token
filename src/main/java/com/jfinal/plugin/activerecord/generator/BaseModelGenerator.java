...
public class BaseModelGenerator {
	...
	protected String importTemplate =
		...
		"import com.jfinal.plugin.activerecord.generator.LocalModel;%n%n";
	protected String classDefineTemplate =
		"/**%n" +
		" * Generated by JFinal, do not modify this file.%n" +
		" */%n" +
		"@SuppressWarnings(\"serial\")%n" +
		"public abstract class %s<M extends %s<M>> extends LocalModel<M> implements IBean {%n%n";
	protected String setterTemplate =
			"\tpublic void %s(%s %s) {%n" +
				"\t\tset(\"%s\", %s);%n" +
			"\t}%n%n";
	protected String setterTemplateChain =
			"\tpublic %s<M> %s(%s %s) {%n" +
				"\t\tset(\"%s\", %s);%n" +
				"\t\treturn this;%n" +
			"\t}%n%n";
...
//			genSetMethodName(columnMeta, ret);
	...
	protected void genSetMethodName(TableMeta tableMeta,ColumnMeta columnMeta, StringBuilder ret) {
		...
		String setter = String.format(setterTemplateChain,tableMeta.baseModelName,setterMethodName, columnMeta.javaType, argName, columnMeta.name, argName);
		ret.append(setter);
	}
